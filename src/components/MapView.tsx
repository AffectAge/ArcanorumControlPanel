...
export default function MapView({
  ...
  useEffect(() => {
    ...
    let lastHoveredProvinceId: string | undefined;
    const handleMouseMove = (event: Event) => {
      const target = event.target as Element | null;
      if (!target) return;
      const path = target.closest('path');
      const provinceId = path?.getAttribute('data-province') || undefined;
      if (provinceId === lastHoveredProvinceId) return;
      lastHoveredProvinceId = provinceId;
  ...
  return (
    <div className="absolute inset-0">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 bottom-2 -translate-y-[0.1rem] z-30">
        ...
        <div className="relative rounded-2xl border border-white/10 bg-[#0b111b] p-2.5 shadow-2xl">
          <div className="flex items-center justify-center">
            <div className="grid grid-flow-col grid-rows-2 place-content-center justify-items-center gap-2">
            {layers.map((layer) => {
